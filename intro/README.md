...
| [AUTOMATION_SPEC.md](./AUTOMATION_SPEC.md) | **For automation agents.** Full spec: every page, every API endpoint (request/response shapes), all UI flows, data-testid reference, error codes, known gotchas, and a recommended test execution order. |
...
| [components/core-service.md](./components/core-service.md) | Domain — projects/suites/cases/runs/results/dumps, dashboard, export. |
| [components/frontend.md](./components/frontend.md) | React SPA — routes, pages, components, design system, theming. |
...
`AUTOMATION_SPEC.md` for the full feature/API inventory, then the relevant
`components/*.md` for the area you're changing. Each component doc lists files,
endpoints, env vars, and gotchas.
